(() => {
  ...
  async function startRun(promptId, lobbyId, promptStart, promptEnd, language) {
    ...
    if (promptId == null) {
      ...
      endpoint = `/api/sprints/${promptId}/runs`;
    else
      endpoint = `/api/lobbys/${lobbyId}/prompts/${promptId}/runs`;
  ...
  async function submitRun(promptId, lobbyId, runId, startTime, endTime, finished, path) {
    ...
    let endpoint = null;
    if (promptId == null)
      endpoint = `/api/quick_runs/runs/${runId}`;
    else if (lobbyId == null)
      endpoint = `/api/sprints/${promptId}/runs/${runId}`;
    else
      endpoint = `/api/lobbys/${lobbyId}/prompts/${promptId}/runs/${runId}`;
  ...
  function submitLocalQuickRun(promptStart, promptEnd, runId, startTime, endTime, finished, path, language) {
    ...
    data["play_time"] = (endTime - startTime) / 1e3 - totalloadtime;
    const key = location;
  ...
  // static/js/pages/play.js
  var PROMPT_ID = serverData["prompt_id"] || null;
  var PROMPT_START = serverData["prompt_start"] || null;
  var PROMPT_END = serverData["prompt_end"] || null;
  var LOBBY_ID = serverData["lobby_id"] || null;
  var IS_SCROLL_ON = serverData["scroll"] || null;
  var LANGUAGE = serverData["lang"];
  ...
  var app = new Vue({
    ...
    methods: {
      ...
      async start() {
        this.countdownTime = (Date.now() - this.startTime) / 1e3;
        ...
        this.timerInterval = setInterval(() => {
          ...
          this.elapsed = (this.milliseconds + this.savedMilliseconds) / 1e3;
        }, 10);
        if (this.isScroll) {
