(() => {
  ...
  // static/js/modules/game/marathon/marathonPrompts.js
  var MarathonPrompts = {
    props: [
      "prompts",
      "username"
  ...
  var AutocompleteInput = {
    ...
    methods: {
      async setAutocomplete(timeCalled) {
        ...
        if (this.lastInputTime === timeCalled)
          this.open();
        else
          this.clearList();
      },
      ...
      setHighlightIndex(index2) {
        this.highlightIndex = index2;
      }
    },
    template: `
  ...
  async function getGeneratedPrompt(difficulty = 1e4, num = 1) {
  ...
  // static/js/modules/customPlay.js
  var CustomPlay = {
    components: {
  ...
  async function updateAnonymousRun2(runId) {
    const reqBody = {
      "run_id": parseInt(runId)
    };
    try {
  ...
  async function uploadLocalMarathons() {
    ...
    for (let runId of runs) {
      try {
        await updateAnonymousRun2(runId);
        runIds.push(runId);
      } catch (e) {
        console.log(e);
      }
  ...
  function generateStreakText(data) {
    ...
    let disp = "";
    if (data["streak"] <= 5)
      disp = `You currently have a \u{1F525}<b> ${data["streak"]} day streak </b>\u{1F525}, keep it up!`;
    else if (data["streak"] <= 10)
      disp = `Good to see you again! You currently have a \u{1F525}<b> ${data["streak"]} day streak </b>\u{1F525}, impressive!`;
    else if (data["streak"] <= 25)
      disp = `Woah, \u{1F525}<b> ${data["streak"]} day streak </b>\u{1F525} and going strong!!`;
    else
      `Didn't think you'd make it this far, but you did! \u{1F525}<b> ${data["streak"]} day streak </b>\u{1F525}!!`;
  ...
  // static/js/modules/lobby/utils.js
