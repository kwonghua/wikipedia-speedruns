(() => {
  ...
  // static/js/modules/wikipediaAPI/util.js
  async function getArticle(page, isMobile = false, lang = "en", revisionDate = null) {
    ...
    if (revisionDate) {
      let revisionurl = `https://${lang}.wikipedia.org/w/api.php?action=query&format=json&origin=*&prop=revisions&redirects=1&titles=${page}&formatversion=2&rvdir=older&rvprop=timestamp|ids&rvlimit=1&rvstart=${revisionDate}`;
      const resp2 = await fetch(revisionurl, { mode: "cors" });
      const body2 = await resp2.json();
      if ("error" in body2) {
        return null;
      }
      const rev_id = body2["query"]["pages"][0]["revisions"][0]["revid"];
      url += `&oldid=${rev_id}`;
    } else {
      url += `&page=${page}`;
    }
  ...
  // static/js/modules/game/marathon/runs.js
  ...
  // static/js/modules/game/tips.js
  function getRandTip() {
    const tips = [
  ...
  // static/js/modules/game/countdown.js
  var CountdownTimer = {
    props: [
      "startArticle",
      "endArticle",
    ...
    data: function() {
      return {
        countdownDuration: 8e3,
        countdownRemaining: 8e3,
        tip: "",
        started: false
      };
    },
    mounted: async function() {
      ...
      const promise1 = new Promise((resolve) => {
        const x = setInterval(() => {
          ...
          if (this.countdownRemaining <= -500) {
            resolve();
            clearInterval(x);
          }
        }, 50);
      ...
      const promise2 = new Promise(
        (r) => document.getElementById("start-btn").addEventListener("click", r, { once: true })
      );
  ...
  // static/js/modules/game/marathon/help.js
  var MarathonHelp = {
    methods: {
      closeHelp: function() {
  ...
  // static/js/modules/game/marathon/finish.js
  var FinishPage = {
    props: [
      "forfeited",
      "startArticle",
    ...
    methods: {
      ...
      generateResults: function(event) {
        let rank = "";
        if (this.numCheckpointsVisited == 0)
          rank = "bruh \u{1F480}";
        else if (this.numCheckpointsVisited <= 3)
          rank = "Wiki Noob ";
        else if (this.numCheckpointsVisited <= 6)
          rank = "Wiki Apprentice";
        else if (this.numCheckpointsVisited <= 9)
          rank = "Wiki Enjoyer";
        else if (this.numCheckpointsVisited <= 14)
          rank = "Winner";
        else if (this.numCheckpointsVisited <= 20)
          rank = "Tryhard";
        else if (this.numCheckpointsVisited <= 35)
          rank = "Overachiever";
        else
          rank = "We didn't plan for anyone to get this far";
        return `WikiSpeedruns Marathon ${this.promptId}
${this.startArticle}
${this.numCheckpointsVisited} \u{1F6A9}
Status: ${rank}`;
      },
    ...
    template: `<div v-cloak class="text-left">
        <div v-if="!saved">
  ...
  // static/js/modules/game/articleRenderer.js
  ...
  function hideElements(frame) {
    ...
    for (let i = 0; i < hide.length; i++) {
      let elements2 = document.getElementsByClassName(hide[i]);
  ...
  function stripNamespaceLinks(frame) {
    frame.querySelectorAll("a").forEach((linkEl) => {
      if (linkEl.hasAttribute("href")) {
        if (linkEl.getAttribute("href").startsWith("/wiki/File:") || linkEl.getAttribute("href").startsWith("/wiki/Wikipedia:") || linkEl.getAttribute("href").startsWith("/wiki/Category:") || linkEl.getAttribute("href").startsWith("/wiki/Help:") || linkEl.getAttribute("href").endsWith("&redlink=1")) {
  ...
  // static/js/modules/game/pagePreview.js
  var PagePreview = {
    props: [
      "lang"
    ],
  ...
  // static/js/modules/localStorage/localStorageMarathon.js
  ...
  // static/js/pages/marathon.js
  var confetti2 = require_confetti();
  var PROMPT_ID = serverData["prompt_id"];
  var load_save = serverData["load_save"] === "1";
  var app = new Vue({
    delimiters: ["[[", "]]"],
    el: "#app",
    components: {
    ...
    methods: {
      ...
      async start() {
        ...
        this.timerInterval = setInterval(() => {
          const seconds = (Date.now() - this.startTime + this.lastTime) / 1e3;
  ...
  function conf() {
    var myConfetti = confetti2.create(document.getElementById("confettiCanvas"), {
      resize: true
    });
    myConfetti({
      particleCount: 100,
