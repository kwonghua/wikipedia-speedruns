(() => {
  ...
  // static/js/pages/admin/categoryLabeler.js
  async function getRandomArticles() {
    const url = "https://en.wikipedia.org/w/api.php?action=query&origin=*&format=json&list=random&rnlimit=15";
  ...
  var app = new Vue({
    ...
    methods: {
      submit: async function() {
        ...
        for (let i = 0; i < this.categories.length; i++) {
          if (document.getElementById("primary_" + String(i)).checked) {
            ...
            primaryFound = true;
          } else if (document.getElementById("secondary_" + String(i)).checked) {
      ...
      updateWithCatName: async function(name) {
        const parse2 = await getArticlesUnderCategory(name);
        await new Promise((r) => setTimeout(r, 300));
        console.log(parse2["articles"].length);
        console.log(name);
        if (parse2["articles"].length < 3) {
          return false;
        }
        this.categoryName = name;
        this.subarticles = parse2["articles"];
        this.subcategories = parse2["subcats"];
        return true;
      },
 ...
 */
//# sourceMappingURL=categoryLabeler.js.map
